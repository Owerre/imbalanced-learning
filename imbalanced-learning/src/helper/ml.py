...
class SupervisedModels:
    ...
    def plot_auc_ap_svm(
        ...
        auc_list = [pd.Series(0.0, index = range(
            len(C_list))) for _ in range(len(gamma_list)
            )
        ]
        ap_list = [pd.Series(0.0, index = range(
            len(C_list))) for _ in range(len(gamma_list)
            )
        ]
    ...
    def plot_auc_ap_lr(self, X_train, y_train, cv_fold = None):
        ...
        auc_list = [
            pd.Series(0.0, 
            index=range(len(C_list))) for _ in range(len(class_wgt_list)
            )
        ]
        ap_list = [
            pd.Series(0.0, 
    ...
    def plot_roc_pr_curves(
        ...
        plt.title('PR Curve for the {}-Fold Cross-Validation Training Set'.format(
            cv_fold)
        )
        plt.legend(loc='best')
    ...
    def plot_aucroc_aucpr(
        ...
        plt.plot([0.75, 1.001], [0.75, 1.001], 'k--', linewidth = 0.5)
        plt.axis([0.75,1.01,0.75,1.01])
        plt.xticks(np.arange(0.76,1.02,0.04))
        plt.yticks(np.arange(0.76,1.02,0.04))
